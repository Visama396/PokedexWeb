...
export default function PokedexInput({ pokedex = [], language = 'es' }) {
	...
	useEffect(() => {
		...
		const filtered = pokedex.filter(({ pokemon, species }) => {
			const localizedName = species.names.find(n => n.language.name === language)?.name || pokemon.name

			return localizedName.toLowerCase().startsWith(normalizedInput)
		})

		setAutoCompletedPokemon(filtered.slice(0, 10))
	}, [inputValue])
