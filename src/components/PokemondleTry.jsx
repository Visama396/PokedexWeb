...
export default function PokemondleTry({ pokemon, pokeTry, language }) {
	...
	return (
		<tr>
			<td className='size-[8rem] rounded-md font-bold' style={{ backgroundColor: compareTypes(pokeTryData.types, pokemon.types, 0) }}>{pokeTryData.types[0]}</td>
			<td className='size-[8rem] rounded-md font-bold' style={{ backgroundColor: compareTypes(pokeTryData.types, pokemon.types, 1) }}>{pokeTryData.types[1] ?? '-'}</td>
			<td className='size-[8rem] rounded-md' style={{ backgroundColor: pokeTryData.height - pokemon.height === 0 ? COLORS.green : COLORS.red }}><p className='font-bold'>{pokeTryData.height / 10} m</p><div className='flex justify-center'>{pokemon.height === pokeTryData.height ? '' : pokemon.height > pokeTryData.height ? <ArrowUp className='size-4 md:size-6' /> : <ArrowDown className='size-4 md:size-6' />}</div></td>
			<td className='size-[8rem] rounded-md' style={{ backgroundColor: pokeTryData.weight - pokemon.weight === 0 ? COLORS.green : COLORS.red }}><p className='font-bold'>{pokeTryData.weight / 10} kg</p><p><div className='flex justify-center'>{pokemon.weight === pokeTryData.weight ? '' : pokemon.weight > pokeTryData.weight ? <ArrowUp className='size-4 md:size-6' /> : <ArrowDown className='size-4 md:size-6' />}</div></p></td>
			...
			<td className='size-[8rem] rounded-md font-bold' style={{ backgroundColor: compareAbilities(pokeTryData.abilities, pokemon.abilities, 1) }}>{pokeTryData.abilities[1] ?? '-'}</td>
			<td className='size-[8rem] rounded-md font-bold' style={{ backgroundColor: compareAbilities(pokeTryData.abilities, pokemon.abilities, 2) }}>{pokeTryData.abilities[2] ?? '-'}</td>
			<td className='size-[8rem] rounded-md' style={{ backgroundColor: pokeTryData.baseStats - pokemon.baseStats === 0 ? COLORS.green : COLORS.red }}><p className='font-bold'>{pokeTryData.baseStats}</p><div className='flex justify-center'>{pokemon.baseStats === pokeTryData.baseStats ? '' : pokemon.baseStats > pokeTryData.baseStats ? <ArrowUp className='size-4 md:size-6' /> : <ArrowDown className='size-4 md:size-6' />}</div></td>
			<td className='size-[8rem] rounded-md' style={{ backgroundColor: translateGenToNum(pokeTryData.generation) === translateGenToNum(pokemon.generation) ? COLORS.green : COLORS.red }}><p className='font-bold'>{pokeTryData.generation}</p><div className='flex justify-center'>{translateGenToNum(pokeTryData.generation) === translateGenToNum(pokemon.generation) ? '' : translateGenToNum(pokeTryData.generation) < translateGenToNum(pokemon.generation) ? <ArrowUp className='size-4 md:size-6' /> : <ArrowDown className='size-4 md:size-6' />}</div></td>
			<td className='size-[8rem] rounded-md'><a href={'/' + pokeTryData.name.toLowerCase()} target='_blank' rel='noopener noreferrer'><img src={pokeTryData.sprite} title={pokeTryData.name} alt={pokeTryData.name} /></a></td>
		</tr>
	)
}
