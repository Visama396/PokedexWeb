...
export default function PokemondleTry({ pokemon, pokeTry, language }) {
	...
	return (
		<tr className='text-xs xl:text-base'>
			<td className='min-w-16 min-h-16 size-16 xl:min-w-32 xl:min-h-32 xl:size-32 rounded-md' style={{ backgroundColor: compareTypes(pokeTryData.types, pokemon.types, 0) }}>{pokeTryData.types[0]}</td>
			<td className='min-w-16 min-h-16 xl:min-w-32 xl:min-h-32 rounded-md' style={{ backgroundColor: compareTypes(pokeTryData.types, pokemon.types, 1) }}>{pokeTryData.types[1] ?? '-'}</td>
			<td className='min-w-16 min-h-16 xl:min-w-32 xl:min-h-32 rounded-md' style={{ backgroundColor: pokeTryData.height - pokemon.height === 0 ? COLORS.green : COLORS.red }}><p>{pokeTryData.height / 10} m</p><div className='flex justify-center'>{pokemon.height === pokeTryData.height ? '' : pokemon.height > pokeTryData.height ? <ArrowUp className='size-6' /> : <ArrowDown className='size-6' />}</div></td>
			<td className='min-w-16 min-h-16 xl:min-w-32 xl:min-h-32 rounded-md' style={{ backgroundColor: pokeTryData.weight - pokemon.weight === 0 ? COLORS.green : COLORS.red }}><p>{pokeTryData.weight / 10} kg</p><p><div className='flex justify-center'>{pokemon.weight === pokeTryData.weight ? '' : pokemon.weight > pokeTryData.weight ? <ArrowUp className='size-6' /> : <ArrowDown className='size-6' />}</div></p></td>
			<td className='min-w-16 min-h-16 xl:min-w-32 xl:min-h-32 rounded-md' style={{ backgroundColor: compareAbilities(pokeTryData.abilities, pokemon.abilities, 0) }}><p>{pokeTryData.abilities[0]}</p></td>
			<td className='min-w-16 min-h-16 xl:min-w-32 xl:min-h-32 rounded-md' style={{ backgroundColor: compareAbilities(pokeTryData.abilities, pokemon.abilities, 1) }}><p>{pokeTryData.abilities[1] ?? '-'}</p></td>
			<td className='min-w-16 min-h-16 xl:min-w-32 xl:min-h-32 rounded-md' style={{ backgroundColor: compareAbilities(pokeTryData.abilities, pokemon.abilities, 2) }}><p>{pokeTryData.abilities[2] ?? '-'}</p></td>
			<td className='min-w-16 min-h-16 xl:min-w-32 xl:min-h-32 rounded-md' style={{ backgroundColor: pokeTryData.baseStats - pokemon.baseStats === 0 ? COLORS.green : COLORS.red }}><p>{pokeTryData.baseStats}</p><div className='flex justify-center'>{pokemon.baseStats === pokeTryData.baseStats ? '' : pokemon.baseStats > pokeTryData.baseStats ? <ArrowUp className='size-6' /> : <ArrowDown className='size-6' />}</div></td>
			<td className='min-w-16 min-h-16 xl:min-w-32 xl:min-h-32 rounded-md' style={{ backgroundColor: translateGenToNum(pokeTryData.generation) === translateGenToNum(pokemon.generation) ? COLORS.green : COLORS.red }}><p>{pokeTryData.generation}</p><div className='flex justify-center'>{translateGenToNum(pokeTryData.generation) === translateGenToNum(pokemon.generation) ? '' : translateGenToNum(pokeTryData.generation) < translateGenToNum(pokemon.generation) ? <ArrowUp className='size-6' /> : <ArrowDown className='size-6' />}</div></td>
			<td className='min-w-16 min-h-16 xl:min-w-32 xl:min-h-32 rounded-md'><a href={'/' + pokeTryData.name.toLowerCase()} target='_blank' rel='noopener noreferrer'><img src={pokeTryData.sprite} title={pokeTryData.name} alt={pokeTryData.name} /></a></td>
		</tr>
	)
}
